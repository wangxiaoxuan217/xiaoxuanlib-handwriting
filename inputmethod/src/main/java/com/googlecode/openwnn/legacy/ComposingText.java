...
public class ComposingText
{
    ...
    private void modifyUpper(int layer, int mod_from, int mod_len, int org_len)
    {
        ...
        {
            /*
             * if there is no element on above layer, add a element includes whole elements of the lower layer.
             */
        ...
        else
        {
            ...
            StrSegment ss = new StrSegment(toString(layer, mod_from, mod_to), mod_from, mod_to);
            strUplayer.add(0, ss);
        ...
        for (int lv = 0; lv < MAX_LAYER; lv++)
        {
            ...
            else
            {
                ...
                if (boundary_from != fromL[lv - 1] || boundary_to != toL[lv - 1])
                {
                    ...
                    boundary_to -= diff;
                    StrSegment[] tmp =
                        new StrSegment[] {(new StrSegment(toString(lv - 1), boundary_from, boundary_to))};
