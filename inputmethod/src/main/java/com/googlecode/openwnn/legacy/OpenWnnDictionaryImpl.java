...
public class OpenWnnDictionaryImpl implements WnnDictionary
{
    ...
    protected static final String NORMAL_QUERY = "select distinct " + COLUMN_NAME_STROKE + "," + COLUMN_NAME_CANDIDATE
        + "," + COLUMN_NAME_POS_LEFT + "," + COLUMN_NAME_POS_RIGHT + "," + COLUMN_NAME_TYPE + " from " + TABLE_NAME_DIC
        + " where %s order by " + COLUMN_NAME_TYPE + " DESC, %s";
    
    protected static final String LINK_QUERY = "select distinct " + COLUMN_NAME_STROKE + "," + COLUMN_NAME_CANDIDATE
        + "," + COLUMN_NAME_POS_LEFT + "," + COLUMN_NAME_POS_RIGHT + "," + COLUMN_NAME_TYPE + " from " + TABLE_NAME_DIC
        + " where %s = ? and %s = ? and %s order by " + COLUMN_NAME_TYPE + " DESC, %s";
    ...
    public OpenWnnDictionaryImpl(String dicLibPath, String dicFilePath)
    {
        ...
        if (this.mWnnWork != 0 && dicFilePath != null)
        {
            ...
            String queryFullBaseString = OpenWnnDictionaryImplJni
                .createQueryStringBase(this.mWnnWork, MAX_LENGTH_OF_QUERY, MAX_PATTERN_OF_APPROX, COLUMN_NAME_STROKE);
            
            String queryFastBaseString = OpenWnnDictionaryImplJni
                .createQueryStringBase(this.mWnnWork, FAST_QUERY_LENGTH, MAX_PATTERN_OF_APPROX, COLUMN_NAME_STROKE);
            
            mExactQuerySqlOrderByFreq = String.format(NORMAL_QUERY,
                String.format("%s=?", COLUMN_NAME_STROKE),
                String.format("%s DESC", COLUMN_NAME_ID));
            
            mExactQuerySqlOrderByKey =
                String.format(NORMAL_QUERY, String.format("%s=?", COLUMN_NAME_STROKE), COLUMN_NAME_STROKE);
            
            mFullPrefixQuerySqlOrderByFreq =
                String.format(NORMAL_QUERY, queryFullBaseString, String.format("%s DESC", COLUMN_NAME_ID));
            
            mFastPrefixQuerySqlOrderByFreq =
                String.format(NORMAL_QUERY, queryFastBaseString, String.format("%s DESC", COLUMN_NAME_ID));
            
            mFullPrefixQuerySqlOrderByKey = String.format(NORMAL_QUERY, queryFullBaseString, COLUMN_NAME_STROKE);
            
            mFastPrefixQuerySqlOrderByKey = String.format(NORMAL_QUERY, queryFastBaseString, COLUMN_NAME_STROKE);
            
            mFullLinkQuerySqlOrderByFreq = String.format(LINK_QUERY,
                COLUMN_NAME_PREVIOUS_STROKE,
                COLUMN_NAME_PREVIOUS_CANDIDATE,
                queryFullBaseString,
                String.format("%s DESC", COLUMN_NAME_ID));
            
            mFastLinkQuerySqlOrderByFreq = String.format(LINK_QUERY,
                COLUMN_NAME_PREVIOUS_STROKE,
                COLUMN_NAME_PREVIOUS_CANDIDATE,
                queryFastBaseString,
                String.format("%s DESC", COLUMN_NAME_ID));
            
            mFullLinkQuerySqlOrderByKey = String.format(LINK_QUERY,
                COLUMN_NAME_PREVIOUS_STROKE,
                COLUMN_NAME_PREVIOUS_CANDIDATE,
                queryFullBaseString,
                COLUMN_NAME_STROKE);
            
            mFastLinkQuerySqlOrderByKey = String.format(LINK_QUERY,
                COLUMN_NAME_PREVIOUS_STROKE,
                COLUMN_NAME_PREVIOUS_CANDIDATE,
                queryFastBaseString,
                COLUMN_NAME_STROKE);
    ...
    {
        String sqlStr = "create table if not exists " + tableName + " (" + COLUMN_NAME_ID
            + " integer primary key autoincrement, " + COLUMN_NAME_TYPE + " integer, " + COLUMN_NAME_STROKE + " text, "
            + COLUMN_NAME_CANDIDATE + " text, " + COLUMN_NAME_POS_LEFT + " integer, " + COLUMN_NAME_POS_RIGHT
            + " integer, " + COLUMN_NAME_PREVIOUS_STROKE + " text, " + COLUMN_NAME_PREVIOUS_CANDIDATE + " text, "
            + COLUMN_NAME_PREVIOUS_POS_LEFT + " integer, " + COLUMN_NAME_PREVIOUS_POS_RIGHT + " integer)";
        ...
        switch (operation)
        {
            ...
            case WnnDictionary.SEARCH_LINK:
                ...
                if (wnnWord != null)
                {
                    ...
                    String[] queryArgsTemp = OpenWnnDictionaryImplJni
                        .createBindArray(this.mWnnWork, keyString, maxBindsOfQuery, MAX_PATTERN_OF_APPROX);
                ...
                {
                    queryArgs = OpenWnnDictionaryImplJni
                        .createBindArray(this.mWnnWork, keyString, maxBindsOfQuery, MAX_PATTERN_OF_APPROX);
                }
                break;
    ...
    public int searchWord(int operation, int order, String keyString)
    {
        ...
        if (mDbDic != null && (mFrequencyOffsetOfUserDictionary >= 0 || mFrequencyOffsetOfLearnDictionary >= 0))
        {
            try
            {
    ...
    public WnnWord getNextWord(int length)
    {
        if (this.mWnnWork != 0)
        {
            if (mDbDic != null && mDbCursor != null && mCountCursor > 0)
            {
                ...
                {
                    /*
                     * Skip results if that is not contained the type of search or length of stroke is not equal
                     * specified length
                     */
    ...
    public WnnWord[] getUserDictionaryWords()
    {
        if (this.mWnnWork != 0 && mDbDic != null)
        {
            ...
            try
            {
                ...
                cursor = (SQLiteCursor)mDbDic.query(TABLE_NAME_DIC,
    ...
    public int clearUserDictionary()
    {
        ...
        {
            mDbDic
                .execSQL(String.format("delete from %s where %s=%d", TABLE_NAME_DIC, COLUMN_NAME_TYPE, TYPE_NAME_USER));
        }
    ...
    public int clearLearnDictionary()
    {
        ...
        {
            mDbDic.execSQL(
                String.format("delete from %s where %s=%d", TABLE_NAME_DIC, COLUMN_NAME_TYPE, TYPE_NAME_LEARN));
        }
                ...
                try
                {
                    ...
                    for (int index = 0; index < word.length; index++)
                    {
                        if (word[index].stroke.length() > 0 && word[index].stroke.length() <= MAX_STROKE_LENGTH
                            && word[index].candidate.length() > 0
                            && word[index].candidate.length() <= MAX_CANDIDATE_LENGTH)
                        {
    ...
    public int learnWord(WnnWord word, WnnWord previousWord)
    {
        if (mDbDic != null)
        {
            ...
            if (previousWord != null && previousWord.stroke.length() > 0
                && previousWord.stroke.length() <= MAX_STROKE_LENGTH && previousWord.candidate.length() > 0
                && previousWord.candidate.length() <= MAX_CANDIDATE_LENGTH)
            {
            ...
            if (word.stroke.length() > 0 && word.stroke.length() <= MAX_STROKE_LENGTH && word.candidate.length() > 0
                && word.candidate.length() <= MAX_CANDIDATE_LENGTH)
            {
                StringBuilder strokeSQL = new StringBuilder();
                StringBuilder candidateSQL = new StringBuilder();
                ...
                {
                    /*
                     * If a registering space is short, delete the words that contain same stroke and candidate to the
                     * oldest word
                     */
                    mDbDic.beginTransaction();
                    try
                    {
                        ...
                        mDbDic.delete(TABLE_NAME_DIC,
                            String.format("%s=%d and %s=%s and %s=%s",
                                ...
                                TYPE_NAME_LEARN,
                                COLUMN_NAME_STROKE,
                                oldestStrokeSQL.toString(),
                                COLUMN_NAME_CANDIDATE,
                                oldestCandidateSQL.toString()),
                            null);
